...
async function getMenuList() {
  ...
  // TODO 下面的内容开发的时候要删掉
  menuListRef.value = []
  menuListRef.value.push(new AirMenu().setName('首页').setId('1').setPath('/home').setComponent('/index/index'))
  menuListRef.value.push(new AirMenu().setName('用户管理').setId('2').setPath('/user').setComponent('/user/list'))
  menuListRef.value.push(new AirMenu().setName('AirHttp').setId('3').setPath('/demo/http').setComponent('/demo/http'))
  // TODO 上面的内容开发的时候要删掉
