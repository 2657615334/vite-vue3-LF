  ...
  <APanel title="首页" description="这是描述信息">
    ...
    <template #body>
      <div>
        ...
        <br /><br />
        <AInput
          v-model.userSex="userInfo.userSex"
          :entity="UserEntity"
          @change="change"
        />
        <br /><br />
        <AInput
          v-model.userType="userInfo.userType"
          :entity="UserEntity"
          :list="userTypeRecord"
        />
        <br /><br />
        ...
        <br /><br />
        <AInput
          v-model.department="userInfo.department"
          :entity="UserEntity"
          :tree="departmentTreeData"
        />
        <br /><br />
        <AirImage />
        <br /><br />
        ...
        <br /><br />
        <el-button v-loading="loading" type="primary" @click="getById()"
          >测试Loading</el-button
        >
        {{ loading }}
        <br /><br />
...
userTypeRecord.value.push(
  new AirRecord().setKey(UserType.IMPLEMENTER_USER).setLabel("实施人员")
);
userTypeRecord.value.push(
  new AirRecord().setKey(UserType.ROOT_USER).setLabel("超管").setDisabled()
);
userTypeRecord.value.push(new AirRecord().setKey(UserType.OTHER).setLabel("其他人员"));

const departmentTreeData = ref([] as AirRecord[]);

function initTree(): AirRecord[] {
  const list: AirRecord[] = [];
  list.push(new AirRecord().setKey(1).setLabel("测试1"));
  list.push(
    new AirRecord()
      .setKey(2)
      .setLabel("测试2")
      .setChildren([
        new AirRecord().setKey(21).setLabel("测试21"),
        new AirRecord().setKey(22).setLabel("测试22"),
        new AirRecord()
          .setKey(23)
          .setLabel("测试23")
          .setChildren([
            new AirRecord().setKey(231).setLabel("测试231"),
            new AirRecord().setKey(232).setLabel("测试232"),
          ]),
      ])
  );
  list.push(
    new AirRecord()
      ...
      .setLabel("测试3")
      .setChildren([
        new AirRecord().setKey(31).setLabel("测试31"),
        new AirRecord()
          .setKey(32)
          .setLabel("测试32")
          .setChildren([
            new AirRecord().setKey(321).setLabel("测试321"),
            new AirRecord().setKey(322).setLabel("测试322"),
          ]),
        new AirRecord()
          .setKey(33)
          .setLabel("测试33")
          .setChildren([
            new AirRecord().setKey(331).setLabel("测试321"),
            new AirRecord().setKey(332).setLabel("测试322"),
          ]),
      ])
  );
  list.push(new AirRecord().setKey(4).setLabel("测试3"));
  return list;
}
...
async function customAlert() {
  ...
  `)
    ...
    .setWidth(800)
    .show();
  await new AirAlert("带我去好滴哦我去红歌会i哦亲无花果i哦还欠我i哦活该i哦清河湾")
    .setTitle("哈哈哈")
    .success();
}
</script>
