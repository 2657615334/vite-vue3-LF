  ...
  >
    <template #body>
      ...
      >
        ...
        <el-form-item
          :label="UserEntity.prototype.getCustomFieldName('userRemark')"
          prop="userRemark"
        >
          <el-input
...
const rules = AirValidatorHelper.create({
  ...
  age: [
    new AirValidator().toNumber().ifEmpty().show("你必须输入的数字"),
    /* new AirValidator().toNumber()
       ...
       .ifNotGreaterThan(0)
       .show(), */
    new AirValidator().toNumber().ifLessThan(0).show(),
    new AirValidator().toNumber().ifGreaterThan(50).show(),
  ],
});
