  ...
  >
    <template #body>
      <div class="tips">
        ...
        <template v-else>
          <el-result icon="success" title="数据导出成功">
            <template #extra>
              <el-button type="primary" @click="download">下载导出的文件</el-button>
            </template>
          </el-result>
        </template>
      </div>
...
const startLoop = async (fileCode: string) => {
  clearTimeout(loopTimer)
  try {
    const downloadPath: string = await new AirHttp('file/downloadPath').withOutError()
    .post({ fileCode })
    loading.value = false
